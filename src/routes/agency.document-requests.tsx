...
export const Route = createFileRoute("/agency/document-requests")({
  head: () => ({ meta: [{ title: "Document Requests · TalVault" }] }),
  loader: async ({ context }) => {
    await Promise.all([
      context.queryClient.ensureQueryData(listQO),
      context.queryClient.ensureQueryData(talentQO),
    ]);
  },
  errorComponent: ({ error }) => (
    <div className="tvp-card" style={{ padding: 24 }}>
      <h1 className="tvp-h1">Document Requests</h1>
      <p className="tvp-muted">Failed to load: {error.message}</p>
    </div>
  ),
  notFoundComponent: () => <div>Not found</div>,
  component: DocRequestsPage,
});
...
function DocRequestsPage() {
  const qc = useQueryClient();
  const { data: rows } = useSuspenseQuery(listQO);
  const { data: talent } = useSuspenseQuery(talentQO);
  ...
  return (
    <>
      <div className="tvp-topbar" style={{ marginBottom: 12 }}>
        <div>
          <h1 className="tvp-h1 inline-flex items-center gap-2">
            <ClipboardList className="h-6 w-6" />Document Requests
          </h1>
        ...
        <div className="tvp-actions">
          <button className="tvp-primary" onClick={() => setShowNew(true)}>
      ...
      >
        ...
        <div className="tvp-small">
          The Talent Portal (which lets talent submit against requests) is not
          live yet. You can still create and review requests against manager
          uploads — this screen is seed-data-ready and will hook into talent
          submissions once the portal ships.
        </div>
      </div>
